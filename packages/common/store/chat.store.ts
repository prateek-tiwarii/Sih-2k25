...
// Local minimal model/config to avoid depending on @repo/ai
export type Model = { id: string; label: string };
const models: Model[] = [{ id: 'local-llama', label: 'Local Llama 3.2' }];
...
const loadInitialData = async () => {
    ...
    const chatMode: ChatMode = config.chatMode || ChatMode.GEMINI_2_FLASH;
...
// Function to initialize the shared worker (disabled for local-only)
const initializeWorker = () => {
    // Use localStorage-based tab sync only
    initializeTabSync();
};
...
const notifyWorker = (type: string, data: any) => {
    // Always use localStorage-based sync in local-only mode
...
export const useChatStore = create(
    immer<State & Actions>((set, get) => ({
        ...
        fetchRemainingCredits: async () => {
            // local-only: skip credit fetching
            set(state => {
                state.creditLimit.isFetched = true;
                state.creditLimit.remaining = undefined;
                state.creditLimit.maxLimit = undefined;
                state.creditLimit.reset = undefined;
                state.creditLimit.isAuthenticated = false;
            });
        },
        ...
        removeFollowupThreadItems: async (_threadItemId: string) => {
            // local-only: no-op
            return;
        },
