// Local-only mode: remove Clerk and workflow worker dependencies
// Local-only: Clerk removed
...
export const AgentProvider = ({ children }: { children: ReactNode }) => {
    ...
    const isSignedIn = true as const;
    ...
    // Stubbed worker handlers for local streaming
    const startWorkflow = (_: any) => {};
    const abortWorkflow = () => {};
    const _onWorkerEvent = useCallback(
        (data: any) => {
                if (
                    data?.threadId &&
                    data?.threadItemId &&
    ...
    const runAgent = useCallback(
        async (body: any) => {
            console.log('🚀 runAgent: Starting with body:', JSON.stringify(body, null, 2));
            ...
            abortController.signal.addEventListener('abort', () => {
                console.info('🚀 runAgent: Abort controller triggered');
                setIsGenerating(false);
            ...
            try {
                console.log('🚀 runAgent: Calling /api/completion');
                ...
                console.log('🚀 runAgent: Response status:', response.status, response.statusText);

                if (!response.ok) {
                    let errorText = await response.text();
                    console.error('🚀 runAgent: Error response:', errorText);
                ...
                if (!response.body) {
                    console.error('🚀 runAgent: No response body received');
                    throw new Error('No response body received');
                }

                console.log('🚀 runAgent: Starting to read stream...');
